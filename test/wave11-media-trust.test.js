...
test('Wave 11 validates magic bytes, MIME and approved dimensions', () => {
    ...
    assert.equal(validateImageDataUrl('data:image/jpeg;base64,' + pngDataUrl().split(',')[1]).code, 'MEDIA_TYPE_MISMATCH');
    assert.equal(validateImageDataUrl('data:image/svg+xml;base64,PHN2Zy8+').code, 'UNSUPPORTED_MEDIA_TYPE');
});
...
test('Wave 11 consume is one atomic transition with explicit terminal status', async () => {
    ...
    const pool = { query: async () => {
        ...
        return { rows: [{ status: 'consumed', expires_at: new Date(Date.now() + 1000) }] };
    } };
    const result = await consumeImage({ pool, mediaId: 'm1', receiverId: 'u1' });
    assert.deepEqual(result, { ok: false, code: 'MEDIA_ALREADY_CONSUMED' });
});
